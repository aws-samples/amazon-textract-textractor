...
class Table(DocumentEntity):
    ...
    def get_text_and_words(
        ...
        # Plaintext or HTML
            ...
            if temp_config.table_flatten_semi_structured_as_plaintext and self.table_type == TableTypes.SEMI_STRUCTURED:
                text = "<p>"
                if config.table_linearization_format == "html":
                    temp_config.table_prefix = "<p>"
                    temp_config.table_suffix = "</p>"
                else:
                    temp_config.table_prefix = ""
                    temp_config.table_suffix = ""
                temp_config.table_linearization_format = "plaintext"
                temp_config.table_row_prefix = ""
                temp_config.table_row_suffix = ""
                temp_config.table_cell_header_prefix = ""
                temp_config.table_cell_header_suffix = ""
                temp_config.table_cell_prefix = ""
                temp_config.table_cell_suffix = " "
            elif temp_config.table_linearization_format == "html":
                temp_config.table_prefix = "<table>"
                temp_config.table_suffix = "</table>"
                temp_config.table_row_prefix = "<tr>"
                temp_config.table_row_suffix = "</tr>"
                temp_config.table_cell_header_prefix = "<th>"
                temp_config.table_cell_header_suffix = "</th>"
                temp_config.table_cell_prefix = "<td>"
                temp_config.table_cell_suffix = "</td>"
                
            row_offset = 0
            processed_cells = set()
            if temp_config.table_flatten_headers:
                columns = ["" for _ in range(len(rows[0][1]))]
                ...
                for _, row in rows:
                    ...
                    for i, cell in enumerate(row):
                        if (
                            cell not in processed_cells or
                            temp_config.table_duplicate_text_in_merged_cells or
                            temp_config.table_flatten_headers
                        ):
                            if cell.siblings:
                                # This handles the edge case where we are flattening the headers
                                # so we want to duplicate the cell text but only in its first row
                                ...
                                for sib in cell.siblings:
                                    children.extend(sib.children)
                                    processed_cells.add(sib)
                                text, _ = linearize_children(children, config=temp_config, no_new_lines=True)
                                columns[i] += text
                            else:
                                text = cell.get_text(temp_config)
                                columns[i] += text
                        elif temp_config.table_cell_empty_cell_placeholder and temp_config.table_linearization_format != "html":
                            columns[i] += temp_config.table_cell_empty_cell_placeholder
                        else:
                            columns[i] += ""
                            column_spans[i] = 0
                    row_offset += 1
                if any(columns):
                    text += temp_config.table_row_prefix if temp_config.add_prefixes_and_suffixes_in_text else ""
                    for column, column_span in zip(columns, column_spans):
                        if column_span == 0:
                            continue
                        
                        if temp_config.table_linearization_format == "html":
                            prefix = f'<th colspan="{column_span}">'
                        else:
                            prefix = (temp_config.table_cell_header_prefix if temp_config.table_cell_header_prefix else temp_config.table_cell_prefix)
                        # We don't have any rowspan logic here as the flattened header will have rowspan=1 always.
                        text += (
                            (
                                prefix + 
                                (column or temp_config.table_cell_empty_cell_placeholder) +
                                (temp_config.table_cell_header_suffix if temp_config.table_cell_header_suffix else temp_config.table_cell_suffix)
                            )
                            if temp_config.add_prefixes_and_suffixes_in_text or temp_config.table_linearization_format == "html" else
                            (column or temp_config.table_cell_empty_cell_placeholder)
                        )
                        text += temp_config.table_column_separator
                    text += (temp_config.table_row_suffix if temp_config.add_prefixes_and_suffixes_in_text else "")
                    text += temp_config.table_row_separator
            ...
            for _, row in rows[row_offset:]:
                ...
                for cell in sorted(row, key=lambda c: c.col_index):
                    ...
                    first_row, first_col, last_row, last_col = cell._get_merged_cell_range()
                    if cell in processed_cells and temp_config.table_linearization_format == "html":
                        continue
                    
                    # Siblings includes the current cell
                    if cell.siblings:
                        children = []
                        if (cell.col_index == first_col and cell.row_index == first_row) or temp_config.table_duplicate_text_in_merged_cells:
                            for sib in cell.siblings:
                                children.extend(sib.children)
                                processed_cells.add(sib)
                            cell_text, _ = linearize_children(children, config=temp_config, no_new_lines=True)
                        elif cell.row_index == first_row and temp_config.table_cell_left_merge_cell_placeholder:
                            cell_text = temp_config.table_cell_left_merge_cell_placeholder
                        elif cell.col_index == first_col and temp_config.table_cell_top_merge_cell_placeholder:
                            cell_text = temp_config.table_cell_top_merge_cell_placeholder
                        elif cell.col_index != first_col and cell.row_index != first_row and temp_config.table_cell_cross_merge_cell_placeholder:
                            cell_text = temp_config.table_cell_cross_merge_cell_placeholder
                        else:
                            cell_text = ""
                            
                    # This is a regular cell
                    else:
                        cell_text, _ = cell.get_text_and_words(temp_config)
                    
                    print(temp_config.table_linearization_format)
                    if temp_config.table_linearization_format == "html":
                        if cell.is_column_header:
                            prefix = (
                                '<th' +
                        ...
                        prefix = temp_config.table_cell_header_prefix if temp_config.table_cell_header_prefix and cell.is_column_header else temp_config.table_cell_prefix
                        
                    text += (
                        (
                            prefix +
                            # Removes trailing whitespace in cell_text
                            (cell_text.strip() or temp_config.table_cell_empty_cell_placeholder) +
                            (temp_config.table_cell_header_suffix if temp_config.table_cell_header_suffix and cell.is_column_header else temp_config.table_cell_suffix)
                        )
                        if temp_config.add_prefixes_and_suffixes_in_text else
                        (cell_text or temp_config.table_cell_empty_cell_placeholder)
                    ) 
                    text += temp_config.table_column_separator
                if text and text[-1] == temp_config.table_column_separator:
                    text = text[:-1]
                text += (temp_config.table_row_suffix if temp_config.add_prefixes_and_suffixes_in_text else "")
                text += temp_config.table_row_separator
                
        if temp_config.table_add_title_as_caption and self.title:
            text += "<caption>" + self.title.get_text() + "</caption>"
            
        text += (temp_config.table_suffix if temp_config.add_prefixes_and_suffixes_in_text else "")
